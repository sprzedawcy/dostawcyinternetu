...
interface Props {
  offer: any;
  onAddressComplete: (address: { miejscowosc: string; ulica: string; nr: string; simc: string; miejscowoscSlug: string }) => void;
}
...
export default function FullAddressSearch({ offer, onAddressComplete }: Props) {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      ...
      try {
        ...
        console.error('Blad szukania miejscowosci:', err);
      }
  ...
  // Szukaj ulic
  useEffect(() => {
    if (!selectedCity?.simc || !hasStreets || streetQuery.length < 2) {
      setStreets([]);
      return;
    }
    ...
    try {
      ...
      if (isOfferAvailable) {
        ...
        const newAdresParam = encodeURIComponent(
          `${newAddress.miejscowosc}|${newAddress.ulica}|${newAddress.nr}|${miejscowoscSlug}|${selectedCity.simc}`
        );
  ...
  const handleGoToResults = () => {
    // Przekieruj do strony głównej z parametrami wyszukiwania
    const id_ulicy = selectedStreet?.id_ulicy || '00000';
    ...
    <div className="bg-white rounded-2xl shadow-sm overflow-hidden border-[3px] border-blue-500" id="kontakt">
      <div className="p-5 bg-gradient-to-r from-blue-500 to-blue-600 text-white">
        <h2 className="text-xl font-bold mb-1">
          Sprawdz dostepnosc oferty
        </h2>
        <p className="text-blue-100 text-sm">
          Podaj adres, zeby sprawdzic czy {offer.nazwa} jest dostepna
        </p>
      </div>
                  ...
                  <div className="absolute z-50 w-full mt-2 bg-white border-2 border-gray-300 rounded-xl shadow-2xl max-h-60 overflow-y-auto">
                    {streets.map((street, index) => (
                      <button
                        ...
                        className="w-full p-3 text-left hover:bg-blue-100 border-b border-gray-200 last:border-0 font-medium text-gray-900"
                      >
                        {street.ulica}
                      </button>
                    ))}
